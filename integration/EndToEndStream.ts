...
async function main() {
  ...
  await streamManager.startStream(erc20.address, bobAddress, oneEther.mul(20000), timestamp + 60, timestamp + oneHour);
  ...
  await treasury.viewAvailableBalance(erc20.address, bobAddress).then((balance: BigNumber) => {
    expect(balance).to.eq(oneEther);
