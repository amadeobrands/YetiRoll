...
xdescribe("Multiple Recipient Stream", () => {
  ...
  it("Should accept an array of addresses and create a stream to each with splitting the deposit in an equal proportion", async () => {
    ...
    await multipleRecipient
      .getStream(1, charlie.address)
      .then((stream: any) => expect(stream.recipient).to.eq(charlie.address));
    await multipleRecipient
      .getStream(1, dennis.address)
      .then((stream: any) => expect(stream.recipient).to.eq(dennis.address));
    await multipleRecipient
      .getStream(1, ethan.address)
      .then((stream: any) => expect(stream.recipient).to.eq(ethan.address));
