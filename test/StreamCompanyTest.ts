...
describe("Stream Employee", () => {
  ...
  it("It should allow the creation of new employees given the calling address has EMPLOYER_ROLE", async () => {
    await streamCompany
      .employees(alice.address)
      .then((address) => expect(address).to.eq(NULL_ADDRESS));

    await streamCompany.createEmployee(alice.address, 1);

    await streamCompany
      .employees(alice.address)
      .then((address) => expect(address).to.not.eq(NULL_ADDRESS));
  });
  it("It should disallow the creation of new employees if the calling address does not have the EMPLOYER_ROLE", async () => {
    ...
    await expect(streamCompanyAliceAddress.createEmployee(alice.address, 100))
      .to.be.reverted;
  });
  ...
  it("It should only allow withdrawal if there is sufficient balance in the pool", async () => {
    ...
    const aliceContract = await streamCompany
      .employees(alice.address)
      .then(
    ...
    await provider
      .getBalance(streamCompany.address)
      .then((balance) => expect(balance).to.be.eq(900));
  });
});
