...
const EmployeeForm = (props) => {
  ...
  const createEmployee = () => {
    company.createEmployee(alice, 100).then((block) => {
  ...
  const displayAddress = () => {
    return <div>Sending address: {alice}</div>;
  };

  const displayEmployee = () => {
    if (undefined !== hasEmployee) {
      return (
        <PausableStream company={company} provider={provider} alice={alice} />
      );
    }
  };
  ...
  return (
    <div>
      <form onSubmit={(event) => event.preventDefault()}>
        <input
          type="submit"
          onClick={useCallback(() => createEmployee(), [alice])}
        />
      </form>
      {displayAddress()}
      {displayEmployee()}
